...
# 3. Remove the first two rows (as they contain column headers and other unnecessary information).
data_clean <- data[-c(1, 2)]

# 4. Check and remove any empty rows.
data_clean <- data_clean[data_clean != ""]

# 5. Use regular expressions to split the content of each line.
# 对于每一行，使用正则表达式将 "line_number" 与后面的字段分开
data_split <- strsplit(data_clean, '" "', perl = TRUE)

# 6. Perform further processing on the split data: remove quotation marks and split the comma-separated fields.
